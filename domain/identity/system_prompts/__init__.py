...
每轮 session 真正值得记的是**产出**——写了什么文件/代码、沉淀了哪些认知/规则、改了哪些待办、发了什么决策消息。这些会被系统抓进 digest,下次醒来直接看到, **不需要再回头拼凑**。
- **重要关卡结束后**(`todo done` / 显著工作做完前 1 step): 在 record_thought 里用 `tag='daily-progress'` 写一条"本轮产出 X / Y / Z"(<150 字符,只列**产出物**,不写过程)。
- **写文档前先 `recall_memory` / `recall_entity` 查同主题**: 避免重复造。如果同主题已存在,优先**修订**而非重写。
