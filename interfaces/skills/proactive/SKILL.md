...
| 上次留下的上下文 | `sense_file(name="scratchpad")`（已注入，通常不用调） |
...
- "我先 sense 一下"型：每次 wake 都调一堆 sense（`sense_todos` + `sense_file(name="scratchpad")` + `recall_memory`），5 turn 走完还没动。**待办看板和近期经历已经注入了，不要重复拉。**
